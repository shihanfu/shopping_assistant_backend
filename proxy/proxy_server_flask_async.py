...
Uses Flask 2.0+ native async support.
"""
...
logger = logging.getLogger("ProxyServerFlaskAsync")
...
connections_lock = threading.Lock()  # Note: Using threading.Lock instead of asyncio.Lock
...
def startup():
    ...
    logger.info("=== Starting Flask Async Proxy Server ===")
    logger.info(f"Server will listen on port {SERVER_LISTEN_PORT}")
    logger.info(f"Chunk size: {CHUNK_SIZE} bytes")

    # Start cleanup thread (not async task)
    cleanup_thread = threading.Thread(target=cleanup_old_connections_sync, daemon=True)
...
async def handle_connection_create():
    ...
    try:
        ...
        with connections_lock:  # Using synchronous lock
    ...
    try:
        ...
        with connections_lock:
            ...
            if is_final:
                ...
                # Note: We need to run the async function in a way Flask can handle
                asyncio.create_task(forward_request(conn))
        ...
        with connections_lock:
            ...
            if chunk_index == 0:
                ...
                if not has_more:
                    ...
                    # Schedule cleanup after a delay using threading.Timer
                    cleanup_timer = threading.Timer(30.0, cleanup_connection_delayed, args=[connection_id])
                    cleanup_timer.start()
...
def cleanup_connection_delayed(connection_id: str):
    """Clean up connection after delay (synchronous version)"""
    with connections_lock:
...
def cleanup_old_connections_sync():
    """Periodically clean up old connections (synchronous version)"""
...
def run():
    """Run the Flask async proxy server"""
    print(f"Starting Flask async proxy server on port {SERVER_LISTEN_PORT}")
    print("This server should be accessible through the AWS SigV4 proxy")

    # Note: Flask async requires Python 3.7+ and specific ASGI server for full async benefits
    # For development, we can use the built-in server, but for production, use hypercorn or similar
    app.run(
        host="0.0.0.0",
